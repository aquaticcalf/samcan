...
main :: proc() {
    ...
    assert(storage.save_autosave(path, &original), "could not write the autosave")
    assert(storage.has_autosave(path), "autosave was not discoverable")
    recovery, recovery_ok := storage.load(storage.autosave_path(path))
    assert(recovery_ok, "could not load the autosave")
    assert(len(recovery.elements) == len(original.elements), "autosave element count did not round-trip")
    document.destroy(&recovery)
    assert(storage.clear_autosave(path), "could not clear the autosave")
    assert(!storage.has_autosave(path), "autosave was not cleared")
    _ = os.remove(path)
